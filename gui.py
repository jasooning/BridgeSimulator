"""
Section Builder (PyQt6) – With Deletion & Bottom-Left Coordinates

Features:
- Add Rectangle, Circle, Polygon
- Delete shapes
- Position (0,0) at bottom-left
- Rectangle position determined by bottom-left corner
- Tree view with composite groups
- Property inspector
- Compute area, centroid, moments
- Import/Export JSON
...
class SectionBuilderMain(QMainWindow):
    def __init__(self):
        ...
        btn_add_rect = QPushButton('Add Rectangle'); btn_add_rect.clicked.connect(self.add_rectangle)
        btn_add_circle = QPushButton('Add Circle'); btn_add_circle.clicked.connect(self.add_circle)
        btn_add_poly = QPushButton('Start Polygon'); btn_add_poly.clicked.connect(self.start_polygon_mode)
        btn_delete = QPushButton('Delete Selected'); btn_delete.clicked.connect(self.delete_selected)
        btn_group = QPushButton('Group Selected'); btn_group.clicked.connect(self.group_selected)
        btn_ungroup = QPushButton('Ungroup Selected'); btn_ungroup.clicked.connect(self.ungroup_selected)
        btn_export = QPushButton('Export JSON'); btn_export.clicked.connect(self.export_json)
        btn_import = QPushButton('Import JSON'); btn_import.clicked.connect(self.import_json)
        btn_compute = QPushButton('Compute Properties'); btn_compute.clicked.connect(self.compute_properties)

        for b in [btn_add_rect, btn_add_circle, btn_add_poly, btn_delete, btn_group, btn_ungroup, btn_export, btn_import, btn_compute]:
        ...
        file_menu.addAction(ACTION := QAction('Export JSON', self)); ACTION.triggered.connect(self.export_json)
        file_menu.addAction(ACTION2 := QAction('Import JSON', self)); ACTION2.triggered.connect(self.import_json)
        help_menu = menubar.addMenu('Help')
        help_menu.addAction(ACTION3 := QAction('Instructions', self)); ACTION3.triggered.connect(self.show_help)

    # ---------- Shape creation ----------
    def add_rectangle(self):
        item = RectangleItem(120, 60, 'Rectangle')
        item.setPos(0, 0)  # bottom-left
        self.scene.addItem(item)
        self.add_tree_item(item)

    def add_circle(self):
        item = CircleItem(40, 'Circle')
        item.setPos(0, 0)
        self.scene.addItem(item)
        self.add_tree_item(item)
    ...
    def finish_polygon(self):
        if len(self.current_poly_points) < 3:
            ...
            self.cancel_polygon()
            return
        poly_item = PolygonItem(self.current_poly_points, 'Polygon')
    ...
    def delete_selected(self):
        ...
        if not items:
            return
        for it in items:
            tree_item = self.find_tree_item_for_graphics(it)
            if tree_item:
    ...
    def import_json(self):
        ...
        for d in data:
            t = d.get('type')
            if t == 'rectangle':
                it = RectangleItem(d.get('w',100), d.get('h',50), d.get('name','Rectangle'))
                it.from_dict(d)
                self.scene.addItem(it)
                self.add_tree_item(it)
            elif t == 'circle':
                it = CircleItem(d.get('r',30), d.get('name','Circle'))
                it.from_dict(d)
                self.scene.addItem(it)
                self.add_tree_item(it)
            elif t == 'polygon':
                pts = d.get('points', [])
                qpts = [QPointF(x,y) for (x,y) in pts]
                it = PolygonItem(qpts, d.get('name','Polygon'))
                it.from_dict(d)
                self.scene.addItem(it)
                self.add_tree_item(it)
    ...
    def compute_properties(self):
        ...
        polys = []
        for s in sel:
            if isinstance(s, RectangleItem): polys.append(s.shape_polygon())
            elif isinstance(s, CircleItem): polys.append(s.shape_polygon())
            elif isinstance(s, PolygonItem): polys.append(s.to_point_list())
        total_area = 0; cx_num = 0; cy_num = 0; Ixx_total=0; Iyy_total=0; Ixy_total=0
        for p in polys:
        ...
        if total_area==0: QMessageBox.warning(self,'Compute','Zero total area'); return
        cx = cx_num / total_area; cy = cy_num / total_area
    ...
    def on_tree_selection_changed(self):
        ...
        if not items: self.clear_properties(); return
        tree_item = items[0]
        g = tree_item.data(0, Qt.ItemDataRole.UserRole)
        if not g: self.clear_properties(); return
    ...
    def eventFilter(self, watched, event):
        if watched is self.view.viewport():
            if event.type() == QEvent.Type.MouseButtonPress:
                ...
                if self.creating_polygon:
                    self.finish_polygon(); return True
            elif event.type() == QEvent.Type.Wheel:
                delta = event.angleDelta().y(); factor = 1.0 + (delta / 1200.0)
                self.scale *= factor
    ...
    def show_help(self):
        text = (
            "Instructions:\n"
            "- Add shapes using buttons\n"
            "- Select shapes to move or edit\n"
            "- Rectangle bottom-left = position\n"
            "- Delete shapes with 'Delete Selected'\n"
            "- Group/Ungroup shapes in tree\n"
            "- Compute properties\n"
            "- Export/Import JSON"
        )
